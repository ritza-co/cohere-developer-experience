...
In addition to writing custom tool functions for the model, we can also convert existing functions to tools Cohere models can use with the `@tool` decorator. 
...
Let's now interact the agent we just defined. We'll start with a question that requires using a predefined tool from Langchain:
...
Let's now ask a question that requires the large language model to use a custom tool: 
...
We see in the output the model making a call to our custom-defined function, followed by a call to the LangChain web search function. It then generates the final answer based on the acquired information. 
...
We see in the output how the agent manages to make the relevant tool calls in the correct sequence to generate the output. 
